...
Report which analyses are current vs stale/missing, then proceed with only the
...
Create the analysis directory if needed (`mkdir -p analysis`). For each
analysis that needs updating, read the prompt file from
`~/.dotfiles/etc/ai-prompts/` and follow its instructions completely. Each
prompt contains its own output format, metadata header, and update strategy.

Work through each level sequentially. Do not start a level until all analyses
in the previous level are complete.
