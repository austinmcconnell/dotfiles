...
find . -name "*model*" -o -name "*entity*" -o -name "*schema*" \
  | grep -E "\.(py|js|ts|rb|java|go)$" | head -20

# Search for ORM and database patterns
grep -r -i "class.*model\|table\|schema\|migration" \
  --include="*.py" --include="*.js" --include="*.ts" \
  --include="*.rb" --include="*.java" . | head -20
...
1. Update the analysis with new schema insights while preserving valuable context
