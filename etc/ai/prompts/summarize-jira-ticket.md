...
Fetch all linked/related tickets as well. For each linked ticket, note the link type (e.g.,
"blocks", "is blocked by", "relates to", "is caused by").
...
For linked tickets, include a section at the top noting which parent ticket linked to it and the
relationship type, so the connection is clear when reading the file standalone.
