...
1. New options that replace workarounds or verbose patterns in my current config
1. Features I'm not using that would benefit my workflow (rebase-heavy, linear history, multi-repo,
   remote work across 2 machines)
...
Skip cosmetic changes. For each recommendation, cite the git version that introduced it. Prioritize
things where a newer feature is strictly better regardless of other choices.
