...
to subagents using the temp-file assembly pattern (see below and the `country-relocation-research`
...
1. If subagents wrote temp files (`.tmp-<topic>.md` + `.tmp-<topic>-sources.yaml`) for a single
   ...
   assembled output. See the `country-relocation-research` skill for the full pattern.
