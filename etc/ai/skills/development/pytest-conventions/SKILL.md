...
description: pytest conventions for fixtures, factories, mocking, parametrize, assertions, and test organization. Use when writing or reviewing pytest tests, test fixtures, or test factories.
---

# Pytest Conventions

The proactive rule for *when* a change needs tests lives in the `testing-standards` steering doc
(resident, so it fires without being asked). This skill covers the mechanics once you're already
writing a test.
